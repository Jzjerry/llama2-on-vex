...
void malloc_run_state(RunState* s, Config* p) {
    ...
    printf("Allocate Cache Size = %d * %d * %d = %d!\n", 
        p->n_layers, p->seq_len, kv_dim, p->n_layers * p->seq_len * kv_dim);
...
void read_checkpoint(Config* config, TransformerWeights* weights,
    ...
    // if (!model_bin) { printf("Couldn't open model!\n"); exit(EXIT_FAILURE); }
    // read in the config header
    if (memcpy(config, model_bin, sizeof(Config)) == NULL) { exit(EXIT_FAILURE); }
    // negative vocab size is hacky way of signaling unshared weights. bit yikes.
    int shared_weights = config->vocab_size > 0 ? 1 : 0;
    config->vocab_size = abs(config->vocab_size);
    *file_size = _binary_bin_model_bin_end - _binary_bin_model_bin_start;
...
void build_transformer(Transformer *t) {
    ...
    printf("Loaded model with %d layers, %d heads, %d dim\n",
           t->config.n_layers, t->config.n_heads, t->config.dim);
...
float* forward(Transformer* transformer, int token, int pos) {
    ...
    for(unsigned long long l = 0; l < p->n_layers; l++) {
        // printf("[Forwarding Layer %d/%d]\n", (int)l, p->n_layers);
        // attention rmsnorm
        // printf("[rmsnorm start]\n");
        ...
        // printf("[q mamuls start]\n");
        matmul(s->q, s->xb, w->wq + l*dim*dim, dim, dim);
        // printf("[k mamuls start]\n");
        matmul(s->k, s->xb, w->wk + l*dim*kv_dim, dim, kv_dim);
        // printf("[v mamuls start]\n");
        ...
        for (int i = 0; i < dim; i+=2) {
            ...
            float val = pos * freq;
            // printf("[RoPE Cos and Sin for val=%d]\n", (int)val);
            float fcr = cosf(val);
            float fci = sinf(val);
        ...
        // printf("[multi-head attention start]\n");
        ...
        // printf("[ffn rmsnorm start]\n");
        // ffn rmsnorm
        rmsnorm(s->xb, x, w->rms_ffn_weight + l*dim, dim);

        // Now for FFN in PyTorch we have: self.w2(F.silu(self.w1(x)) * self.w3(x))
        // first calculate self.w1(x) and self.w3(x)
        // printf("[ffn matmul start]\n");
        ...
        // printf("[ffn output matmul start]\n");
...
void generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, char *prompt, int steps) {
    ...
    while (pos < steps) {
        printf("[pos %d: start forwarding]\n", pos);
        // forward the transformer to get logits for the next token
        float* logits = forward(transformer, token, pos);
        printf("[pos %d: finish forwarding]\n", pos);
